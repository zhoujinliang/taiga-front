...
class FiltersMixin {
    selectFilter(name, value, load) {
        ...
        if ((params[name] !== undefined) && (name !== "page")) {
            let existing = _.map(taiga.toString(params[name]).split(","), x => trim(x));
    ...
    unselectFilter(name, value, load) {
        ...
        let parsedValues = _.map(taiga.toString(params[name]).split(","), x => trim(x));
        let newValues = _.reject(parsedValues, x => x === taiga.toString(value));
    ...
    storeFilters(projectSlug, params, filtersHashSuffix) {
        ...
        let hash = taiga.generateHash([projectSlug, ns]);
