...
export class UserTimelineService extends Service {
    ...
    _attachExtraInfoToTimelineEntry(timeline, event, type) {
        const title = this.userTimelineItemTitle.getTitle(timeline, event, type);
    ...
    _parseTimeline(response) {
        ...
        response.get("data").forEach((item) => {
            let newItem;
            const event = this._parseEventType(item.get("event_type"));
    ...
    _addEntyAttributes(item) {
        const event = this._parseEventType(item.get("event_type"));
        const type = this.userTimelineItemType.getType(item, event);
